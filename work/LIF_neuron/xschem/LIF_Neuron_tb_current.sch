...
* Average current
let IDD = -1*i(VSP1)
let IBUF = -1*i(VSP2)*0.5

meas tran I_avg AVG IDD from=11n to=tstop
let P_avg = abs(I_avg*Vhigh)
meas tran I_buf AVG IBUF from=11n to=tstop
let P_buf = abs(I_buf*Vhigh)
...
device=resistor
m=1}
C {devices/vsource.sym} 220 -290 0 0 {name=VSP1 value=\{VDD\}}
...
device=resistor
m=1}
C {devices/vsource.sym} 100 -290 0 0 {name=VSP2 value=\{VDD\}}
...
footprint=1206
device="ceramic capacitor"}
C {lab_wire.sym} 1880 -430 3 0 {name=p30 sig_type=std_logic lab=VSS}
C {devices/capa.sym} 1880 -470 0 0 {name=CCOMP
m=1
value=\{Cload\}
footprint=1206
device="ceramic capacitor"}
C {lab_wire.sym} 1520 -580 0 1 {name=p3 sig_type=std_logic lab=COMP}
C {devices/code_shown.sym} 935 -332.5 0 0 {name=PEX only_toplevel=true
format="tcleval( @value )"
value="
.include /foss/designs/chipathon_2025/designs/ihp-sg13g2/LIF_neuron/netlist/pex/LIF_neuron.spice
.include /foss/designs/chipathon_2025/designs/ihp-sg13g2/gate_buff_pad/netlist/pex/gate_buff_pad.spice
"}
